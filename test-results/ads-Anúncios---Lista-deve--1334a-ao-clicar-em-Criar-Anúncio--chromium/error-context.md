...
- Name: ads.spec.ts >> Anúncios - Lista >> deve navegar para criação ao clicar em "Criar Anúncio"
- Location: src\ads.spec.ts:145:7
...
Call log:
  - waiting for locator('a:has-text("Criar Anúncio")')
    - locator resolved to 2 elements. Proceeding with the first one: <a href="/ads" routerlinkactive="active" _ngcontent-ng-c438088021="" class="submenu-item ng-star-inserted"> Criar Anúncio </a>
  - attempting click action
    - waiting for element to be visible, enabled and stable
    - element is visible, enabled and stable
    - scrolling into view if needed
    - done scrolling
    - <div _ngcontent-ng-c438088021="" class="menu-item ng-star-inserted">…</div> intercepts pointer events
  - retrying click action
    - waiting for element to be visible, enabled and stable
    - element is visible, enabled and stable
    - scrolling into view if needed
    - done scrolling
    - <div class="menu-item-core" _ngcontent-ng-c438088021="">…</div> from <div tabindex="0" role="button" _ngcontent-ng-c438088021="" class="menu-parent ng-star-inserted">…</div> subtree intercepts pointer events
  - retrying click action
    - waiting 20ms
    - waiting for element to be visible, enabled and stable
    - element is visible, enabled and stable
  ...
  2 × retrying click action
      ...
      - scrolling into view if needed
      - done scrolling
      - <a href="/config" routerlinkactive="active" _ngcontent-ng-c438088021="" class="menu-simple ng-star-inserted">…</a> from <div _ngcontent-ng-c438088021="" class="menu-item ng-star-inserted">…</div> subtree intercepts pointer events
...
> 148 |     await page.click('a:has-text("Criar Anúncio")');
      |                ^ Error: page.click: Test timeout of 30000ms exceeded.
  149 |     await page.waitForURL('**/ads');
  150 |     await expect(page).toHaveURL(/\/ads$/);
  151 |   });
  152 | 
  153 |   // ─── Modal de Exclusão ────────────────────────────────────────────
  154 | 
  155 |   test('deve abrir modal de exclusão ao clicar em delete', async ({
  156 |     page,
  157 |   }) => {
  158 |     await page.waitForTimeout(3000);
  159 |     const deleteButtons = page.locator('.btn-delete');
  160 | 
  161 |     if ((await deleteButtons.count()) > 0) {
  162 |       await deleteButtons.first().click();
  163 | 
  164 |       await expect(page.locator('.g-modal-overlay')).toBeVisible();
  165 |       await expect(page.locator('text=Confirmar Exclusão')).toBeVisible();
  166 |     }
  167 |   });
  168 | 
  169 |   test('deve fechar modal de exclusão ao cancelar', async ({ page }) => {
  170 |     await page.waitForTimeout(3000);
  171 |     const deleteButtons = page.locator('.btn-delete');
  172 | 
  173 |     if ((await deleteButtons.count()) > 0) {
  174 |       await deleteButtons.first().click();
  175 |       await expect(page.locator('.g-modal-overlay')).toBeVisible();
  176 |       await page.click('button.btn-cancel');
  177 |       await expect(page.locator('.g-modal-overlay')).not.toBeVisible();
  178 |     }
  179 |   });
  180 | 
  181 |   // ─── Seleção Total ────────────────────────────────────────────────
  182 | 
  183 |   test('deve selecionar/desselecionar todos os anúncios', async ({
  184 |     page,
  185 |   }) => {
  186 |     await page.waitForTimeout(3000);
  187 |     const adsRows = page.locator('.ads-row');
  188 | 
  189 |     if ((await adsRows.count()) > 0) {
  190 |       const selectAllCheckbox = page.locator(
  191 |         '.ads-list-header input[type="checkbox"]',
  192 |       );
  193 |       await selectAllCheckbox.check();
  194 |       await page.waitForTimeout(300);
  195 | 
  196 |       const bulkActions = page.locator('.g-bulk-actions');
  197 |       await expect(bulkActions).toHaveClass(/active/);
  198 | 
  199 |       await selectAllCheckbox.uncheck();
  200 |       await page.waitForTimeout(300);
  201 |       await expect(bulkActions).not.toHaveClass(/active/);
  202 |     }
  203 |   });
  204 | });
  205 | 
