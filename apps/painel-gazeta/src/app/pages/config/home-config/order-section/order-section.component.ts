...
export class OrderSectionComponent implements OnInit {
  ...
  toggleSectionTitle(sectionId: string) {
    const updatedSections = this.sections().map((section) =>
      section.id === sectionId
        ? { ...section, showTitle: !section.showTitle }
        : section,
    );
    this.sections.set(updatedSections);
    this.emitSectionsChange();
  ...
  saveSections() {
    ...
    this.configService
      .bulkUpdateSections(sectionsToSave)
      .pipe(first())
      .subscribe({
        next: (savedSections) => {
          ...
          this.alertService.success(
            'Sucesso',
            'Ordenação das seções salva com sucesso!',
          );
        },
        error: (error) => {
          this.loading.set(false);
          ...
          if (error.status === 404) {
            ...
            this.alertService.error(
              'Erro',
              'Erro ao salvar ordenação das seções',
            );
