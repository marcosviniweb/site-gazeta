...
export class SocialMediaComponent implements OnInit, OnDestroy {
  ...
  form = this.fb.group({
    ...
    twitter: [
      '',
      [Validators.pattern(/^https?:\/\/(www\.)?(twitter\.com|x\.com)\/.+/)],
    ],
  ...
  errorMessage = {
    instagram: {
      pattern:
        'Digite uma URL válida do Instagram (ex: https://instagram.com/usuario)',
    },
    facebook: {
      pattern:
        'Digite uma URL válida do Facebook (ex: https://facebook.com/pagina)',
    },
    youtube: {
      pattern:
        'Digite uma URL válida do YouTube (ex: https://youtube.com/c/canal)',
    },
    linkedin: {
      pattern:
        'Digite uma URL válida do LinkedIn (ex: https://linkedin.com/company/empresa)',
    },
    twitter: {
      pattern:
        'Digite uma URL válida do Twitter/X (ex: https://twitter.com/usuario)',
    },
    tiktok: {
      pattern:
        'Digite uma URL válida do TikTok (ex: https://tiktok.com/@usuario)',
    },
    whatsapp: {
      pattern:
      ...
      this.apiService.createSocialMedia(formValue).subscribe({
        next: (data) => {
          this.alertService.success(
            'Sucesso',
            'Configurações de redes sociais salvas com sucesso!',
          );
        },
        error: (error) => {
          ...
          this.alertService.error(
            'Erro',
            'Erro ao salvar configurações de redes sociais',
          );
        },
      });
