...
export class CategoryFormComponent {
  ...
  private colorUpdateEffect = effect(() => {
    ...
    if (!editing) {
      ...
      if (
        used.includes(currentColor) ||
        (currentColor === '#3b82f6' && used.length > 0)
      ) {
  ...
  isColorAvailable = computed(() => {
    ...
    if (editingCategory) {
      const usedWithoutCurrent = used.filter(
        (color) => color !== editingCategory.color,
      );
  ...
  private selectAvailableColor(): void {
    const used = this.usedColors();
    const availableColor = this.predefinedColors.find(
